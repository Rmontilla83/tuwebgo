...
import { ICONO_CANAL, IconMarcador, IconOjo, IconClic, IconUsuario, IconDinero, IconLista } from '@/components/icons'
...
/** Icono del canal, con el nombre accesible para lectores de pantalla. */
  ...
  return (
    <div className="animate-fade-in">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-4 mb-8 stagger">
        <KpiCard label="Sesiones" value={m.totalSessions} sub="visitantes únicos" icon={<IconOjo className="w-[18px] h-[18px]" />} gradient="from-indigo-500 to-violet-600" />
        <KpiCard label="Clics CTA" value={m.ctaClicks} sub={`${clickRate}% de sesiones`} icon={<IconClic className="w-[18px] h-[18px]" />} gradient="from-blue-500 to-cyan-500" />
        <KpiCard label="Leads" value={m.totalLeads} sub={`${leadRate}% de clics`} icon={<IconUsuario className="w-[18px] h-[18px]" />} gradient="from-emerald-500 to-teal-500" />
        <KpiCard label="Revenue" value={`$${m.revenue}`} sub={`${m.wonLeads} ventas`} icon={<IconDinero className="w-[18px] h-[18px]" />} gradient="from-amber-500 to-orange-500" />
      </div>
              ...
              {m.recentLeads.map((lead) => (
                <div key={lead.id} className="bg-[var(--bg)] rounded-xl p-3.5 border border-[var(--border-light)]">
                  ...
                  <div className="flex items-center gap-2 mt-2">
                    <CanalIcono canal={lead.source_channel} />
